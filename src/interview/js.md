...
- 1. 全局执行上下文：全局上下文只有一个，他的取决于宿主环境，在浏览器中，全局上下文就是 window 对象，在 Node 中，全局上下文就是 global 对象。全局上下文在我们遇见脚本的时候就开始创建的，他是随着 js 引擎的初始化开始加载的。
- 2. 函数执行上下文：函数执行上下文是函数被调用时创建的。

执行上下文的概念在 js 中是颇为重要的，变量或函数的执行上下决定了我们可以访问哪些数据以及他的行为。每个执行上下文都关联一个变量对象，变量对象包含了该上下文定义的所有的变量和函数。上下文中所有的代码都在执行完毕后被销毁，包含定义在它上面的所有变量和函数。每个函数的调用都有它自己的上下文，也就是函数执行上下文。当代码执行流入函数时，函数的上下文被推到一个上下文栈中，函数执行完后上下文栈会弹出该函数上下文，并将控制权交给之前的执行上下文。

执行上下文中的代码执行的时候会创建变量对象的一个作用域链。这个作用链决定了各级上下文代码访问变量和函数的顺序。如果是函数执行上下文，则他的活动对象用作变量对象。将上一个变量对象于该活动对象链接起来的链就叫作用域链。
:::
...
1. 解析 HTML 构建 DOM 树
   ...
   - 读取 HTML 文档：浏览器从网络中获取 HTML 文件，通常是通过 HTTP 请求从服务器下载。
   - 词法分析：浏览器将 HTML 文档分解成一系列的标记（tokens），这些标记代表 HTML 元素、属性、文本节点等。
   - 解析标记：浏览器解析这些标记，识别出不同的 HTML 元素及其层次结构。
   - 构建节点：为每个标记创建相应的 DOM 节点。DOM 节点是文档对象模型（DOM）的基本单元，代表文档中的每个元素、属性和文本。
   - 构建 DOM 树：根据 HTML 元素的嵌套关系，将这些节点组织成一棵树结构，称为 DOM 树。DOM 树的根节点是 `<html>` 元素，其他元素根据其在 HTML 文档中的位置成为子节点。
   - 处理脚本和样式：在构建 DOM 树的过程中，浏览器会遇到 `<script>` 和 `<style>` 标签。对于 `<script>` 标签，浏览器可能会暂停 DOM 树的构建，执行脚本，然后继续构建。
   - 如果 在构建过程中遇见`<script>`标签，并没有设置 defer 或 async 属性，则浏览器会立即下载 解析 并执行脚本，然后继续构建 DOM 树。
   - 如果遇见设置了 defer 的脚本，则浏览器会继续构建 DOM 树，同时将脚本添加到队列中，在 DOM 树构建完成后，浏览器会执行队列中的脚本。
   - 如果遇见设置了 async 的脚本，脚本会下载完成后立即解析执行，而且不会等文档解析完成，所以他是乱序的，所以不保证执行顺序。

   对于`<style>`标签，浏览器会解析 CSS 并应用样式。

   - 错误处理：如果 HTML 文档中存在语法错误，浏览器会尝试纠正这些错误，以便继续构建 DOM 树。
...
2. 解析 CSS 构建 CSSOM 树
   ...
   - 处理媒体查询：浏览器会根据当前设备的特性（如屏幕宽度）解析和应用符合条件的媒体查询中的样式。
   - 合并样式：在构建 CSSOM 树的过程中，浏览器会处理样式的层叠（cascading）和继承（inheritance）规则，计算出每个元素的最终样式。
   - CSSOM 树与 DOM 树结合后，浏览器可以计算出每个元素的样式信息，进而进行布局和绘制。构建 CSSOM 树是一个关键步骤，因为它直接影响页面的渲染速度和样式的应用。
3. 将 DOM 树和 CSSOM 树合并成一个渲染树
   - 浏览器将 DOM 树和 CSSOM 树合并成一个渲染树。
   - 渲染树的根节点为 document 对象。
   - 渲染树的节点是 DOM 树和 CSSOM 树的节点。
4. 根据渲染树计算每个节点的几何信息
   - 浏览器根据渲染树计算每个节点的几何信息。
   - 几何信息包括节点的位置、大小、布局等。
5. 将各个节点绘制到屏幕上
