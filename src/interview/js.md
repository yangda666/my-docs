...
1. 解析 HTML 构建 DOM 树
   - 浏览器将 HTML 解析成一个 DOM 树，DOM 树的根节点为 document 对象。
   - 浏览器会解析 HTML 中的标签，构建 DOM 树。
   - 解析过程中，如果遇到 script 标签，会暂停解析过程，优先执行脚本。
   - 解析过程中，如果遇到 link 标签，会暂停解析过程，优先加载 CSS 文件。
   - 解析过程中，如果遇到 img 标签，会暂停解析过程，优先加载图片。
2. 解析 CSS 构建 CSSOM 树
   - 浏览器将 CSS 解析成一个 CSSOM 树，CSSOM 树的根节点为 document 对象。
   - 浏览器会解析 CSS 中的样式，构建 CSSOM 树。
   - 解析过程中，如果遇到 @import 标签，会暂停解析过程，优先加载 CSS 文件。
   - 解析过程中，如果遇到 @media 标签，会暂停解析过程，优先加载 CSS 文件。
